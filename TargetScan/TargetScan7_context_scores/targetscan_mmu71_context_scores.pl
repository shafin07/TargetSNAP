...
#          - Update to include context++ scores (Agarwal et al.)
#
#######################################################################
...
# - Add min/max values from Vikram's Excel file
# - Incorporate more contributions:
# - ORFs lengths
# - ORFs 8me counts
# - UTR offset-6mer counts
# - PCT
# - Add option to print/not print contributions
#
...
$AgarwalParamFile = "Agarwal_2014_parameters.txt";
...
# Produce verbose output? (0=no; 1=yes)
...
$RNAplfold_IN_OUT = "/nfs/targetscan/mmu_70/6_context_scores/RNAplfold_in_out";
...
{
	$contextScoreFieldNum = 28;	# 0-based
	$percentileRankFieldNum = 29;	# 0-based
}
else
{
...
sub readOtherDataFiles
{
	# Get names of other 4 files and then read them

	print STDERR "Reading names of other files from $OtherDataFiles...\n";
	open (OTHER_FILES, $OtherDataFiles) || die "Cannot open $OtherDataFiles for reading: $!";
	my ($orfLengthsFile, $orf8mersFile, $utrOffset6mersFile, $AIRsFile) = <OTHER_FILES>;
	chomp ($orfLengthsFile, $orf8mersFile, $utrOffset6mersFile, $AIRsFile);
	...
	{
		chomp;
		my ($orf, $species, $miRNAfamily, $foo, $count) = split /\t/, $_;
		$foo = "";	# Extra field that should be ignored
		if($useSpecies{$species})
		{
	...
	open (UTR_OFFSET_6MERS, $utrOffset6mersFile) || die "Cannot open $utrOffset6mersFile for reading: $!";
	print STDERR "Reading UTR offset 6mer counts file ($utrOffset6mersFile)...\n";
	while (<UTR_OFFSET_6MERS>)
	{
		chomp;
		my ($utr, $species, $miRNAfamily, $foo1, $foo2, $sites) = split /\t/, $_;
		$foo1 = $foo2 = "";	# Extra field that should be ignored
		if($useSpecies{$species})
		{
			$utrToOffset6merSites{$utr}{$species}{$miRNAfamily} = $sites;
		}
	}
...
sub getAirThisSite
{
	...
	# print STDERR "getAirThisSite ($transcriptID, $speciesID, $siteEnd) => 0\n$msg\nNearest region AIR = $airClosestRegion\n";
	if ( defined($airClosestRegion))
	{
		$airThisRegion = 1.0*$airClosestRegion/100;
	}
	else
	{
		print STDERR "Can't get AIR info for transcript $transcriptID (species $speciesID) for site ending at $siteEnd => setting AIR to 1.\n";
		$airThisRegion = 1;
	}
...
sub getUsage
{
	...
	USAGE:
		$0 miRNA_file UTR_file PredictedTargets_file OtherData_file ContextScoresOutput_file

	EXAMPLE:
		$0 miR_for_context_scores.txt UTR_Sequences_sample.txt targetscan_70_output.txt OtherDataFiles.txt context_scores_70_output.txt

	Required input files:
		miRNA_file       => mature miRNA data [not used by targetscan_70.pl]
		UTR_file         => aligned UTRs (same as for targetscan_70.pl)
		PredictedTargets => output from targetscan_70.pl
		OtherDataFiles   => list of 4 files with other required data
...
sub checkArguments
{
	...
	{
		print STDERR "\n*** You need to supply a name for the OtherDataFiles ***\n";
		print STDERR "$usage";
		exit;
	}
	...
	$OtherDataFiles = $ARGV[3];
	$contextScoreFileOutput = $ARGV[4];
	$contextScoresOutputTemp = "$ARGV[4].tmp";
...
sub getFileFormats
{
	...
	** Required input files:
	...
	3 - PredictedTargets_file => targets predicted by targetscan_70.pl
	
		contains 13 fields, although some fields are ignored
		
		See targetscan_70_output.txt for sample

	4 - OtherData_file      => Filenames with scores that cannot be determined from other files

		contains a list of four files (one per line):
			a. ORF lengths (for ORFs matching 3\' UTRs in UTR_file):     file with SequenceID, species, length
			b. ORF 8mer counts (for ORFs matching 3\' UTRs in UTR_file): file with SequenceID, species, miRNA family ID/name, count
			c. UTR offset-6mer counts:                                  file with SequenceID, species, miRNA family ID/name, count
			d. PCT [format undetermined]
			
		ex:
		ORF_lengths.txt
		ORF_8mer_counts.txt
		ORF_8mer_counts.txt
		PCT_scores_per_site.txt
...
sub addPercentileRanksToOtherData
{
	...
	while (<CONTEXT_SCORES_OUTPUT_TEMP>)
	{
		...
		{
			# print STDERR "Cannot calculate percentile rank from context score *$f[$contextScoreFieldNum]*\n";
		}
...
sub getSA_contribution
{
	...
	my $RNAplfold_outfile = "$RNAplfold_IN_OUT/$speciesID/$transcriptID.${speciesID}_lunp";
	...
	{
		print STDERR "Cannot find RNAplfold output file: ($RNAplfold_outfile).  Using 0 for plfold score.\n";
		$plfold = 0;
	}
...
sub getORF8mer_contribution
{
	...
	if ($orfTo8merCounts{$transcriptID}{$speciesID}{$miRNA_familyID})
	{
		$orf8mer_count = $orfTo8merCounts{$transcriptID}{$speciesID}{$miRNA_familyID};
	}
